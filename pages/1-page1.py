...
#출력 메서드 
st.text('Tushar-Aggarwal.com') #화면에 출력 또는 입력을 받으려면 st.어쩌구 해야함
st.markdown('[Tushar-Aggarwal.com](https://tushar-aggarwal.com)')
st.caption('Success')
st.latex(r''' e^{i\pi} + 1 = 0 ''') #수식을 출력하는 문법
st.write('Supreme Applcations by Tushar Aggarwal') #알아서 
...
#camera_input을 입력, download_button을 입력에 대한 출력으로 사용하려면?
#data = st.camera_input('Click a Snap')
#st.download_button('Download Source data', data)
#이미지,음성 등은 binary로 들어오게 됨

data = st.camera_input('Click a Snap')

if data: # data 변수에 값이 있으면 아래 부분이 동작
#샘플코드로 응용
#with open(data, "rb") as file: #open 명령어는 하드디스크에 있는 파일을 읽어
    st.download_button(
